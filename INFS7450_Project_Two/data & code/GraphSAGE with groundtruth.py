# -*- coding: utf-8 -*-
"""
Created on Tue Jul  7 13:41:43 2020
...
# Generate test graph
G_test_nx = nx.read_edgelist('groundtruth.txt')

nodes_test = G_test_nx.nodes()
feature_vector = [1,1,1,1,1]
node_data_test = pd.DataFrame(
    [feature_vector for i in range(len(nodes_test))],
    index=[node for node in nodes_test])
G_test = sg.StellarGraph.from_networkx(G_test_nx, node_features=node_data_test)

# Generate negative data in test
edges_test_neg = []
i = 0
while i < len(nodes_test):
    node = choice(list(nodes_test))
    target = choice(list(nodes_test))
    if node != target and (node, target) not in G_test_nx.edges():
        edges_test_neg.append((node, target))
        i += 1
